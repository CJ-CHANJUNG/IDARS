...
session.findById("wnd[0]").sendVKey 0
Err.Clear
WScript.Echo "[STEP 3] ✓ Billing document entered and executed"
...
If tree Is Nothing Then
    ...
    WScript.Echo "[STEP 5] Trying to enumerate usr children..."

    ' usr 하위 요소 직접 탐색
    On Error Resume Next
    Set usrContainer = session.findById("wnd[0]/usr")
    If Not usrContainer Is Nothing Then
        WScript.Echo "[STEP 5] usr container found, type: " & usrContainer.Type
        WScript.Echo "[STEP 5] usr ID: " & usrContainer.ID
    Else
        WScript.Echo "[STEP 5] usr container not accessible"
    End If
